                ...
                <div class="flex items-center space-x-4">
                    ...
                    @auth
                        @if(auth()->user()->isAdmin() || auth()->user()->isEditor())
                            <div class="hidden lg:block relative" x-data="{ adminOpen: false }">
                                <button @click="adminOpen = !adminOpen"
                                    class="flex items-center cursor-pointer p-2 text-gray-600 hover:text-red-600 hover:bg-gray-50 rounded-lg transition duration-200"
                                ...
                                <div x-show="adminOpen" @click.away="adminOpen = false"
                                    ...
                                    @if(auth()->user()->isAdmin())
                                        <a href="{{ route('webmaster.dashboard') }}"
                                            class="flex items-center px-4 py-2 text-sm text-gray-700 hover:bg-red-50 hover:text-red-600 transition duration-200">
        ...
        <div class="flex-1 overflow-y-auto">
            ...
            <div class="p-4">
                ...
                <div class="space-y-2">
                    ...
                    @auth
                        @if(auth()->user()->isAdmin() || auth()->user()->isEditor())
                            <a href="{{ auth()->user()->isAdmin() ? route('webmaster.dashboard') : route('editor.dashboard') }}"
                                class="flex items-center justify-between p-3 bg-purple-50 text-purple-700 rounded-xl hover:bg-purple-100 transition duration-200 group border border-purple-200">
                                <div class="flex items-center">
                                    ...
                                    <div>
                                        <p class="text-sm font-semibold">
                                            {{ auth()->user()->isAdmin() ? 'Admin Dashboard' : 'Editor Dashboard' }}
                                        </p>
                                        <p class="text-xs text-purple-600">Manage content</p>
                                    </div>
                                </div>
                                <svg class="w-4 h-4 text-purple-600 opacity-70 group-hover:translate-x-1 transition-transform duration-200"
                                    fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7" />
                                </svg>
                            </a>
        ...
        <div class="p-4 space-y-6">
            ...
            @auth
                @if(auth()->user()->isAdmin() || auth()->user()->isEditor())
                    <div class="space-y-3">
                        <a href="{{ auth()->user()->isAdmin() ? route('webmaster.dashboard') : route('editor.dashboard') }}"
                            class="w-full flex items-center justify-center px-4 py-3 bg-purple-600 text-white text-sm font-medium rounded-lg hover:bg-purple-700 transition duration-200">
                            <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" 
                                    d="M3 12l2-2m0 0l7-7 7 7M5 10v10a1 1 0 001 1h3m10-11l2 2m-2-2v10a1 1 0 01-1 1h-3m-6 0a1 1 0 001-1v-4a1 1 0 011-1h2a1 1 0 011 1v4a1 1 0 001 1m-6 0h6"/>
                            </svg>
                            {{ auth()->user()->role === 'isAdmin' ? 'Admin Dashboard' : 'Editor Dashboard' }}
                        </a>
                    </div>
                @endif
            @endauth
