                    ...
                    <div class="lg:w-1/4 bg-gray-50 dark:bg-gray-700 p-6 border-r border-gray-200 dark:border-gray-600">
                        ...
                        <div class="mb-8">
                            <h1 class="text-2xl font-bold text-gray-900 dark:text-white">
                                {{ $editingId ? 'Edit Sports Career' : 'Add Sports Career' }}
                            </h1>
                            <p class="text-gray-600 dark:text-gray-400 mt-2">
                                {{ $editingId ? 'Update sports career information' : 'Create a new sports career entry' }}
                            </p>
                        </div>
                        ...
                        <form wire:submit="save">
                            ...
                            @if ($currentStep == 1)
                                <div class="space-y-6">
                                    ...
                                    <!-- Person Selection -->
                                    @include('livewire.admin.sports.partials.person-select')
                                    ...
                                    <div>
                                        ...
                                        <input wire:model="sport" list="sports"
                                            ...
                                            placeholder="Select or enter sport" required />
                                        <datalist id="sports">
                                            @foreach ($commonSports as $sport)
                                                <option value="{{ $sport }}">
                                            @endforeach
                                        </datalist>
                                    </div>
                                    ...
                                    <div>
                                        ...
                                        <input wire:model="team" list="teams"
                                            class="block w-full pl-3 pr-10 py-2 text-base border border-gray-300 focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm rounded-md dark:bg-gray-700 dark:border-gray-600 dark:text-white"
                                            placeholder="Select or enter team" required />
                                        <datalist id="teams">
                                            @foreach ($commonTeams as $team)
                                                <option value="{{ $team }}">
                                            @endforeach
                                        </datalist>
                                    </div>
                                    ...
                                    <div>
                                        ...
                                        <div class="relative">
                                            ...
                                            @if ($autoSlug && !empty($sport) && !empty($team) && empty($slug))
                                        ...
                                        @if ($autoSlug)
                                            <p class="mt-1 text-sm text-green-600 dark:text-green-400">
                                                ✓ Slug auto-generates from sport and team
                                            </p>
                                        @else
                            ...
                            @if ($currentStep == 2)
                                <div class="space-y-6">
                                    <div>
                                        <h2 class="text-xl font-bold text-gray-900 dark:text-white mb-2">Career Details
                                        </h2>
                                    ...
                                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                        <!-- Debut Date -->
                                        <x-flux::input label="Debut Date" wire:model="debut_date" type="date"
                                            helper="When they made their professional debut" />

                                        <!-- Retirement Date -->
                                        <x-flux::input label="Retirement Date" wire:model="retirement_date"
                                            type="date" helper="When they retired (leave empty if still active)" />
                                    </div>

                                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                        <!-- Jersey Number -->
                                        <x-flux::input label="Jersey Number" wire:model="jersey_number"
                                            placeholder="e.g., 7, 10, 23" helper="Player's jersey/number" />

                                        <!-- Coach Name -->
                                        <x-flux::input label="Coach Name" wire:model="coach_name"
                                            placeholder="e.g., John Smith" helper="Primary coach or trainer" />
                                    </div>

                                    <!-- International Player -->
                                    <div>
                                        <label class="flex items-center">
                                            <input type="checkbox" wire:model="international_player"
                                            ...
                                            <span class="ml-2 text-sm text-gray-700 dark:text-gray-300">International
                                                Player</span>
                                        </label>
                                        <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">
                                            Check if this athlete has represented their country internationally
                                        </p>
                                    </div>
                                    ...
                                    <div>
                                        <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
                                            Leagues & Tournaments Participated
                                        </label>
                                        <div class="space-y-2">
                                            @foreach ($leagues_participated as $index => $league)
                                                <div class="flex gap-2">
                                                    <x-flux::input
                                                        wire:model="leagues_participated.{{ $index }}"
                                                        placeholder="e.g., IPL, World Cup, Olympics" class="flex-1" />
                                                    @if (count($leagues_participated) > 1)
                                                        <x-flux::button type="button"
                                                            wire:click="removeLeague({{ $index }})"
                                            ...
                                            @endforeach
                                            <x-flux::button type="button" wire:click="addLeague"
                                                class="cursor-pointer" size="sm">
                                                Add League
                                            </x-flux::button>
                                        </div>
                                        <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">
                                            List major leagues, tournaments, and competitions
                                        </p>
                                    </div>
                                </div>
                            @endif
                            ...
                            @if ($currentStep == 3)
                                <div class="space-y-6">
                                    <div>
                                        <h2 class="text-xl font-bold text-gray-900 dark:text-white mb-2">Stats &
                                            Achievements</h2>
                                        <p class="text-gray-600 dark:text-gray-400">Record statistics, achievements,
                                            and notable events.</p>
                                    </div>
                                    ...
                                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                        ...
                                        <div>
                                            ...
                                            <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">
                                                Uncheck if this sports career has ended
                                            </p>
                                        </div>
                                    ...
                                    <div>
                                        ...
                                        <div class="space-y-2">
                                            @foreach ($stats as $index => $stat)
                                                <div class="flex gap-2">
                                                    <x-flux::input wire:model="stats.{{ $index }}"
                                                        placeholder="e.g., 5000 runs, 100 wickets, 50 goals"
                                                        class="flex-1" />
                                                    @if (count($stats) > 1)
                                                        <x-flux::button type="button"
                                            ...
                                            @endforeach
                                            <x-flux::button type="button" wire:click="addStat"
                                                class="cursor-pointer" size="sm">
                                                Add Statistic
                                            </x-flux::button>
                                        </div>
                                        <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">
                                            Key career statistics and records
                                        </p>
                                    </div>
                                    ...
                                    <div>
                                        ...
                                        <div class="space-y-2">
                                            @foreach ($achievements as $index => $achievement)
                                                <div class="flex gap-2">
                                                    <x-flux::input wire:model="achievements.{{ $index }}"
                                                        placeholder="e.g., World Cup Winner, Olympic Gold, MVP Award"
                                                        class="flex-1" />
                                        ...
                                        <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">
                                            Major awards, championships, and accomplishments
                                        </p>
                                    </div>
                                    ...
                                    <div>
                                        ...
                                        <div class="space-y-2">
                                            @foreach ($notable_events as $index => $event)
                                                <div class="flex gap-2">
                                                    <x-flux::input wire:model="notable_events.{{ $index }}"
                                                        placeholder="e.g., Record-breaking performance, Historic match"
                                                        class="flex-1" />
                                                    @if (count($notable_events) > 1)
                                                        <x-flux::button type="button"
                                                            wire:click="removeNotableEvent({{ $index }})"
                                                            variant="danger" size="sm">
                                                            Remove
                                                        </x-flux::button>
                                                    @endif
                                            ...
                                            @endforeach
                                            <x-flux::button type="button" wire:click="addNotableEvent"
                                                class="cursor-pointer" size="sm">
                                                Add Event
                                            </x-flux::button>
                                        </div>
                                        <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">
                                            Memorable matches, performances, and career highlights
                                        </p>
                                    </div>

                                    <!-- Award Selection -->
                                    @include('livewire.admin.sports.partials.award-select')
                                </div>
                            @endif
                            ...
                            <div class="flex justify-between pt-8 border-t border-gray-200 dark:border-gray-600 mt-8">
                                ...
                                @else
                                    <x-flux::button type="submit" class="cursor-pointer">
                                        ✓ {{ $editingId ? 'Update Sports Career' : 'Create Sports Career' }}
                                    </x-flux::button>
                                @endif
                            </div>
                        </form>
