    ...
    <div>
        <div class="p-6 bg-white dark:bg-gray-800 rounded-lg shadow">
            ...
            <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6">
                <div>
                    <h2 class="text-2xl font-bold text-gray-900 dark:text-white">Lesser Known Facts Management</h2>
                    <p class="text-gray-600 dark:text-gray-400">Manage interesting facts and trivia about persons</p>
                </div>
                <a href="{{ route('webmaster.persons.facts.manage') }}"
                ...
                <div class="md:col-span-2">
                    ...
                    <div class="relative">
                        ...
                        <input wire:model.live="search" id="search"
                            ...
                            placeholder="Search facts..." type="search">
                    </div>
                </div>
                            ...
                            <tr class="hover:bg-gray-50 dark:hover:bg-gray-700">
                                <td class="px-6 py-4">
                                    ...
                                    <div class="text-sm text-gray-500 dark:text-gray-400 mt-1">
                                        {{ Str::limit($fact->fact, 100) }}
                                    </div>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <div class="flex items-center">
                                        <div class="flex-shrink-0 h-10 w-10">
                                            @if ($fact->person->profile_image_url)
                                                <img class="h-10 w-10 rounded-full object-cover"
                                                    src="{{ $fact->person->imageSize(60, 60, 100) ?? $fact->person->profile_image_url }}"
                                                    alt="{{ $fact->person->display_name }}">
                                            @else
                                                <div
                                                    class="h-10 w-10 rounded-full bg-gray-200 dark:bg-gray-600 flex items-center justify-center">
                                                    <span class="text-sm font-medium text-gray-500 dark:text-gray-400">
                                                        {{ strtoupper(substr($fact->person->name, 0, 1)) }}
                                                    </span>
                                                </div>
                                            @endif
                                        </div>
                                        <div class="ml-4">
                                            <div class="text-sm font-medium text-gray-900 dark:text-white">
                                                {{ $fact->person->display_name }}
                                            </div>
                                            <div class="text-xs text-gray-500 dark:text-gray-400">
                                                {{ $fact->person->primary_profession ?? 'Professional' }}
                                            </div>
                                        </div>
                                    </div>
                                </td>
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                    <div class="flex justify-end space-x-2">
                                        <a href="{{ route('webmaster.persons.facts.manage', ['id' => $fact->id]) }}"
                                        ...
                                        <button wire:click="deleteFact({{ $fact->id }})"
                                            wire:confirm="Are you sure you want to delete this fact?"
