<div>
    <div class="p-6 bg-white dark:bg-gray-800 rounded-lg shadow">
        ...
        <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6">
            <div>
                <h2 class="text-2xl font-bold text-gray-900 dark:text-white">Feedback Management</h2>
                <p class="text-gray-600 dark:text-gray-400">Review and manage user feedback</p>
            </div>
            ...
            <div>
                ...
                <div class="relative">
                    ...
                    <input wire:model.live="search" id="search"
                        ...
                        placeholder="Search feedback, names, emails..." type="search">
                </div>
            </div>
                        ...
                        <tr class="hover:bg-gray-50 dark:hover:bg-gray-700">
                            <td class="px-6 py-4">
                                <div class="flex items-center">
                                    <div class="flex-shrink-0 h-10 w-10">
                                        @if ($feedback->person?->profile_image_url)
                                            <img class="h-10 w-10 rounded-full object-cover"
                                                src="{{ $feedback->person->imageSize(40, 40, 100) ?? $feedback->person->profile_image_url }}"
                                                alt="{{ $feedback->person->display_name }}">
                                        @else
                                            <div
                                                class="h-10 w-10 rounded-full bg-gray-200 dark:bg-gray-600 flex items-center justify-center">
                                                <span class="text-sm font-medium text-gray-500 dark:text-gray-400">
                                                    {{ strtoupper(substr($feedback->person?->name ?? 'U', 0, 1)) }}
                                                </span>
                                            </div>
                                        @endif
                                    </div>
                            ...
                            <td class="px-6 py-4">
                                <div class="text-sm">
                                    <span @class([
                                        ...
                                        'bg-purple-100 text-purple-800 dark:bg-purple-800 dark:text-purple-100' => $feedback->type === 'suggestion',
                                        'bg-blue-100 text-blue-800 dark:bg-blue-800 dark:text-blue-100' => $feedback->type === 'correction',
                                        'bg-green-100 text-green-800 dark:bg-green-800 dark:text-green-100' => $feedback->type === 'addition',
                                        'bg-yellow-100 text-yellow-800 dark:bg-yellow-800 dark:text-yellow-100' => $feedback->type === 'update',
                                    ])>
                                        {{ ucfirst($feedback->type) }}
                                    </span>
                                </div>
                                <div class="text-sm text-gray-500 dark:text-gray-400 mt-1 line-clamp-2">
                                    {{ $this->getCleanExcerpt($feedback->message, 100) }}
                                </div>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-gray-400">
                                @if ($feedback->isVerified())
                                    ...
                                    <div class="text-xs text-gray-400">
                                        {{ $feedback->email_verified_at?->format('M d, Y') }}
                                    </div>
                                @else
                            ...
                            <td class="px-6 py-4 whitespace-nowrap">
                                <span @class([
                                    ...
                                    'bg-yellow-100 text-yellow-800 dark:bg-yellow-800 dark:text-yellow-100' => $feedback->status === 'pending',
                                    'bg-green-100 text-green-800 dark:bg-green-800 dark:text-green-100' => $feedback->status === 'approved',
                                    'bg-red-100 text-red-800 dark:bg-red-800 dark:text-red-100' => $feedback->status === 'rejected',
                                ])>
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                <div class="flex justify-end space-x-2">
                                    <a href="{{ route('webmaster.feedback.review', $feedback->id) }}"
                                    ...
                                    @if ($feedback->status === 'approved')
                                        <button wire:click="resendApprovalEmail({{ $feedback->id }})"
                                    ...
                                    <button wire:click="deleteFeedback({{ $feedback->id }})"
                                        wire:confirm="Are you sure you want to delete feedback from {{ $feedback->name }}?"
                                ...
                                <div class="flex justify-end space-x-1 mt-2">
                                    @foreach (['pending', 'approved', 'rejected'] as $actionStatus)
                                        @if ($feedback->status !== $actionStatus)
                                            <button wire:click="updateStatus({{ $feedback->id }}, '{{ $actionStatus }}')"
                                                class="inline-flex items-center px-2 py-1 text-xs rounded border cursor-pointer
                                                {{ $actionStatus === 'approved' ? 'border-green-300 text-green-600 bg-green-50 hover:bg-green-100' : '' }}
                                                {{ $actionStatus === 'rejected' ? 'border-red-300 text-red-600 bg-red-50 hover:bg-red-100' : '' }}
                                                {{ $actionStatus === 'pending' ? 'border-yellow-300 text-yellow-600 bg-yellow-50 hover:bg-yellow-100' : '' }}">
                                                {{ ucfirst($actionStatus) }}
                                            </button>
                                        @endif
                                    @endforeach
                                </div>
