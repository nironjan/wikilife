<div>
    <div class="p-6 bg-white dark:bg-gray-800 rounded-lg shadow">
        ...
        <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6">
            <div>
                <h2 class="text-2xl font-bold text-gray-900 dark:text-white">Political Career Management</h2>
                <p class="text-gray-600 dark:text-gray-400">Manage political careers and government positions</p>
            </div>
            <a href="{{ route('webmaster.persons.politician.manage') }}"
        ...
        <div class="grid grid-cols-1 md:grid-cols-5 gap-4 mb-6">
            ...
            <div class="md:col-span-2">
                ...
                <div class="relative">
                    ...
                    <input wire:model.live="search" id="search"
                        ...
                        placeholder="Search politicians, parties, positions..." type="search">
                </div>
            </div>
            ...
            <!-- Office Type Filter -->
            <div>
                <select wire:model.live="officeType"
                    class="block w-full pl-3 pr-10 py-2 text-base border border-gray-300 focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm rounded-md dark:bg-gray-700 dark:border-gray-600 dark:text-white">
                        ...
                        <tr class="hover:bg-gray-50 dark:hover:bg-gray-700">
                            <td class="px-6 py-4">
                                <div class="flex items-center">
                                    <div
                                        class="flex-shrink-0 h-12 w-12 bg-gradient-to-br from-red-500 to-blue-600 rounded-lg flex items-center justify-center">
                                    ...
                                    <div class="ml-4">
                                        <div class="text-sm font-medium text-gray-900 dark:text-white">
                                            {{ $politician->person->display_name ?? 'N/A' }}
                                        </div>
                                ...
                                @if ($politician->office_type)
                                    <div class="mt-1">
                                        <span
                                            ...
                                            {{ $politician->office_type }}
                                        </span>
                                    </div>
                                @endif
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-gray-400">
                                @if ($politician->tenure_start)
                                    {{ $politician->tenure_start->format('M Y') }} -
                                    {{ $politician->tenure_end ? $politician->tenure_end->format('M Y') : 'Present' }}
                                    <div class="text-xs text-gray-400">
                                        {{ $politician->tenure_duration }}
                                    </div>
                                @endif
                                @if ($politician->joining_date)
                                    <div class="text-xs">
                                        Party: {{ $politician->joining_date->format('M Y') }}
                                    </div>
                                @endif
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">
                                <span @class([
                                    ...
                                    'bg-red-100 text-red-800 dark:bg-red-800 dark:text-red-100' => !$politician->is_active,
                                ])>
                                    {{ $politician->is_active ? 'Active' : 'Inactive' }}
                                </span>
                                @if ($politician->is_current)
                                    <div class="mt-1">
                                        <span
                            ...
                            <td class="px-6 py-4 whitespace-nowrap">
                                <!-- Awards Display -->
                                @if ($politician->awards_count > 0)
                                    <div class="mt-1">
                                        <span
                                            class="inline-flex items-center px-2 py-0.5 rounded text-xs font-medium bg-purple-100 text-purple-800 dark:bg-purple-800 dark:text-purple-100">
                                            {{ $politician->awards_count }}
                                            Award{{ $politician->awards_count > 1 ? 's' : '' }}
                                        </span>
                                    </div>
                                @endif
                            </td>
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                <div class="flex justify-end space-x-2">
                                    <a href="{{ route('webmaster.persons.politician.manage', ['id' => $politician->id]) }}"
                                    ...
                                    <button wire:click="toggleStatus({{ $politician->id }})"
                                        ...
                                        {{ $politician->is_active ? 'Deactivate' : 'Activate' }}
                                    </button>
                                    <button wire:click="deletePolitician({{ $politician->id }})"
                                        wire:confirm="Are you sure you want to delete this political career entry?"
                    ...
                    @empty
                        <tr>
                            <td colspan="5" class="px-6 py-4 text-center text-sm text-gray-500 dark:text-gray-400">
                                No political career entries found matching your criteria.
                            </td>
                        </tr>
                    @endforelse
                </tbody>
